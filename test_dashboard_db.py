...
class DashboardDbTests(unittest.TestCase):
    ...
    def test_fetch_trend_rows_returns_active_high_and_cumulative_mitigated_by_scan(self):
        ...
        self.assertIn("LEFT JOIN asm_current_findings active_c", sql)
        self.assertIn("COALESCE(active_c.endpoint_id, '') = COALESCE(active_f.endpoint_id, '')", sql)
        self.assertIn("COALESCE(active_c.check_id, '') = COALESCE(active_f.check_id, '')", sql)
        self.assertIn("COALESCE(active_c.host, '') = COALESCE(active_f.host, '')", sql)
        self.assertIn("COALESCE(active_c.port, -1) = COALESCE(active_f.port, -1)", sql)
        ...
        self.assertIn("LEFT JOIN dashboard_whitelist_effective d ON d.finding_key = c.finding_key", sql)
